...
function SetMoveBehind(string fRI_Var_Int_MoveBehindMobID, int fDistance, int fHealth, int fFallBackPCName)
{
	...
	if ${Actor[id,${fRI_Var_Int_MoveBehindMobID}](exists)}
	{
		...
		RI_Var_String_MoveBehindFallBackPCName:Set[${fFallBackPCName}]
	}
	else
	{
		if ${RI_Var_Bool_Debug}
...
function RIMoveInFront()
{
	if ${RI_Var_Bool_MovingInFront} && ${Actor[id,${RI_Var_Int_MoveInFrontMobID}].Distance2D}<${RI_Var_Int_MoveInFrontDistance} && ${Actor[id,${RI_Var_Int_MoveInFrontMobID}].Health}<=${RI_Var_Int_MoveInFrontHealth} && !${RI_Var_Bool_LockSpotting}
	{
		...
		while ${RI_Var_Bool_MovingInFront} && (${Actor[id,${RI_Var_Int_MoveInFrontMobID}].Distance2D}>${Position.GetMeleeMaxRange[${RI_Var_Int_MoveInFrontMobID}]} || ${Position.Angle[${RI_Var_Int_MoveInFrontMobID}]}<150) && ${RI_Var_Bool_MovingInFront} && ${Actor[id,${RI_Var_Int_MoveInFrontMobID}](exists)} && !${Actor[id,${RI_Var_Int_MoveInFrontMobID}].IsDead} && !${RI_Var_Bool_LockSpotting}
		{
			if ${RI_Var_Bool_Debug}
